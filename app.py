    ...
    if not FONT_PATH.exists():
        ...
        with st.spinner("正在下載中文字型（僅第一次需要）..."):
            for url in FONT_URLS:
                ...
                st.error("字型下載失敗：請確認部署環境可連線外網，或稍後再試。")
                if last_err:
...
def generate_pdf_bytes(
    ...
    story.append(Paragraph("1. 乙方提供之廣告文案、策略建議、報告等成果，甲方於付清全部款項後，得於本案範圍內使用。", styles['number_item']))
