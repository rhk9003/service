...
# =========================================================
# 6) 甲方資訊（可輸入）＋乙方資訊（鎖死只讀）
# =========================================================
st.header("🧾 甲方資訊（委託暨付款方）")
party_a_name = st.text_input("甲方名稱/公司名稱", placeholder="請輸入付款方名稱（公司或個人）")

st.markdown("---")
st.header("👤 乙方資訊（服務執行者，固定）")

st.text_input("乙方名稱", value=PROVIDER_NAME, disabled=True)
col1, col2 = st.columns(2)
with col1:
    st.text_input("銀行代碼", value=BANK_CODE, disabled=True)
with col2:
    st.text_input("帳號", value=ACCOUNT_NUMBER, disabled=True)

st.text_input("銀行名稱", value=BANK_NAME, disabled=True)
...
def generate_pdf_bytes(
    ...
    story.append(Paragraph(f"<b>乙方（服務執行者）：</b>{PROVIDER_NAME}", styles["party"]))
    ...
    story.append(Paragraph("2. 若因平台政策、帳號狀況或其他不可控因素需採替代作業方式（例如：由甲方匯出報表供乙方監控），甲方同意合理配合。", styles["number_item"]))
    ...
    story.append(Paragraph("1. 乙方將盡專業所能優化廣告成效，但投放成效受市場環境、競爭狀況、消費者行為、平台演算法等多重因素影響，乙方不保證特定之轉換率、ROAS 或銷售成果。", styles["number_item"]))
    story.append(Paragraph("2. 因平台政策變更、帳號異常、不可抗力因素等非乙方可控原因導致之廣告中斷或成效下降，乙方不負賠償責任。", styles["number_item"]))
    story.append(Paragraph("3. 甲方提供之素材、商品或服務如違反平台政策或法令規定，導致廣告被拒絕或帳號受處分，乙方不負相關責任。", styles["number_item"]))
    ...
    if payment_option == "17,000元/月（每月付款）":
        story.append(Paragraph("2. 月付方案：已支付之當期費用不予退還。", styles["number_item"]))
    else:
        story.append(Paragraph(
    ...
    signature_data = [
        ["甲方（委託暨付款方）", "", "乙方（服務執行者）"],
        ["", "", ""],
        [f"名稱：{party_a_name}", "", f"名稱：{PROVIDER_NAME}"],
        ["", "", ""],
...
# =========================================================
# 8) 生成：把結果存進 session_state（下載後不消失）
# =========================================================
st.header("✅ 生成")
